...
| `cv` | 计算机视觉（分割/抠图/美学评分） | Vision | 不联网 |
...
curl -LO https://github.com/Eli-Zxh/macai/releases/download/v1.0.0/cv

# 赋予执行权限
chmod +x ocr asr tts cv
...
# CV：前景物体分割，分别导出每个物体
./cv photo.jpg -m foreground --all-instances -o ./out/

# CV：图片美学评分
./cv photo.jpg -m aesthetics -f json
```
...
基于 macOS Vision Framework 的计算机视觉工具。支持人物分割、前景物体分割、图片美学评分。全程本地处理，Apple Silicon Neural Engine 加速。
...
|------|------|------|--------|
| `<图片路径>` | — | 要处理的图片（位置参数） | 必填 |
| `--mode <模式>` | `-m` | `person` 人物分割；`foreground` 前景物体分割；`aesthetics` 美学评分 | `person` |
| `--quality <质量>` | `-q` | 分割质量: `fast` / `balanced` / `accurate`（仅 person/foreground 有效） | `balanced` |
| `--format <格式>` | `-f` | `png` 抠图（透明背景）；`mask` 灰度蒙版；`json` 结构化数据（aesthetics 模式支持 `txt`） | `png` |
| `--output <路径>` | `-o` | 输出文件路径或目录 | 当前目录 |
| `--all-instances` | — | foreground 模式：分别导出每个前景物体 | 关闭（合并导出） |
| `--batch <目录>` | — | 批量处理目录下所有图片 | 无（单文件模式） |
| `--help` | `-h` | 显示帮助 | — |

**支持的图片格式**：JPEG, PNG, TIFF, HEIC, BMP, WebP 等所有 macOS 支持的图片格式。

### 模式说明

| 模式 | 最低版本 | 说明 |
|------|---------|------|
| `person` | macOS 12+ | 检测并分割人物，输出透明背景 PNG 或蒙版 |
| `foreground` | macOS 14+ | 检测所有前景物体，可逐个导出或合并导出 |
| `aesthetics` | macOS 15+ | 评估图片美学质量，输出 -1.0 到 1.0 的评分 |

### 输出示例

**人物分割（png）** — `./cv photo.jpg`：

输出 `photo_person.png`，人物区域保留，背景透明。

**人物分割（mask）** — `./cv photo.jpg -f mask`：

输出 `photo_person_mask.png`，灰度蒙版（白色=人物，黑色=背景）。

**前景分割（逐个导出）** — `./cv photo.jpg -m foreground --all-instances -o ./out/`：

```
处理: photo.jpg (1920x1080, 模式: foreground)
检测到 3 个前景物体
  物体 #1 → ./out/photo_fg_1.png
  物体 #2 → ./out/photo_fg_2.png
  物体 #3 → ./out/photo_fg_3.png
```

**美学评分（json）** — `./cv photo.jpg -m aesthetics -f json`：
...
`overall_score` 范围 -1.0（差）到 1.0（好）。`is_utility` 为 true 表示技术质量好但缺乏美学价值的图片（如截图、收据）。

### 场景示例

```bash
# 证件照抠图
./cv id_photo.jpg -m person -o id_cutout.png

# 批量人物抠图
./cv --batch ./photos/ -m person -o ./cutouts/

# 提取照片中所有物体
./cv scene.jpg -m foreground --all-instances -o ./objects/

# 评估照片质量
./cv photo.jpg -m aesthetics

# 批量筛选高质量照片
./cv --batch ./vacation/ -m aesthetics -f json -o quality.json
```
...
```bash
# 从复杂背景中抠出文字区域，再 OCR
./cv document.jpg -m foreground --all-instances -o ./regions/
./ocr -d ./regions/ -l zh-Hans,en-US
```
...
```bash
# 方法 1：移除隔离属性
xattr -d com.apple.quarantine ocr asr tts cv
...
| CV | 不联网 | Vision Framework 完全本地处理 |
